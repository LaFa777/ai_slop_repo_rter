...
2. Перед фильтрацией каждому article добавляется `composite_id = f"{url}|||{edit_text}"`
3. `wm.filter_new(all_articles, id_key="composite_id")` — возвращает только комбинации
   url+edit_text, которых ещё не было. **На первом запуске** возвращает пустой список (baseline).
   
   **Почему composite_id:** одна и та же страница может сначала выйти как новый релиз (edit пустой),
   а позже — как обновление (edit непустой). Фильтр по одному url пропустил бы второй случай.
   Склейка url+edit_text отличает «новый релиз» от «обновления к той же игре».
4. `wm.save()` — дописывает все текущие composite_id в `seen_ids`, обрезает до 500
